"use client"

import type * as React from "react"

const ChartContainer = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
  return <div className={`relative ${className}`} {...props} />
}

const Chart = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <svg className="h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" {...props}>
      {children}
    </svg>
  )
}

const LineChart = () => {
  return null
}

const Line = ({ dataKey, stroke, name }: { dataKey: string; stroke: string; name: string }) => {
  return null
}

const XAxis = ({ dataKey }: { dataKey: string }) => {
  return null
}

const YAxis = () => {
  return null
}

interface ChartTooltipProps {
  children?: React.ReactNode
}

const ChartTooltip = ({ children }: ChartTooltipProps) => {
  return null
}

const ChartTooltipContent = () => {
  return null
}

export { Chart, Line, LineChart, XAxis, YAxis, ChartContainer, ChartTooltip, ChartTooltipContent }
